...
def partition_dataset():
    ...
    train_set = torch.utils.data.DataLoader(
        dataset, batch_size=bsz, sampler=train_sampler)
...
def run(gpu):
    ...
    for epoch in range(epochs):
        ...
        for data, target in train_set:
            ...
            loss.backward()
            optimizer.step()
        logging.info('Epoch %d Loss %.6f Global batch size %d on %d ranks', 
                     epoch, epoch_loss / num_batches, gbatch_size, dist.get_world_size())
    
    if gpu:
        logging.info("GPU training time: %s", datetime.datetime.now() - time_start)
    else:
        logging.info("CPU training time: %s", datetime.datetime.now() - time_start)
...
if __name__ == "__main__":
    ...
    dist.init_process_group(
        backend='gloo' if not args.gpu else 'nccl',
        init_method='env://',
